...
const extractBriefField = (question: string, label: string) => {
  const line = question
    .split('\n')
...
const buildPlatformIntelligenceAnswer = (question?: string) => {
  const text = question?.trim() || '';
  const isPlatformBrief = text.includes('منصة القسطاس الرقمي') || text.includes('تحليل ذكي من نشاطك');
  if (!isPlatformBrief) return null;

  const title = text
    .split('\n')[0]
    ?.replace('حلل لي هذا التنبيه من منصة القسطاس الرقمي:', '')
    .trim() || 'تنبيه ذكي';
  const context = extractBriefField(text, 'السياق') || 'لا يوجد سياق تفصيلي مرفق.';
  const requestedStep = extractBriefField(text, 'الخطوة المطلوبة') || 'تحديد الخطوة التالية بوضوح.';
  const expected = extractBriefField(text, 'أريد إجابة عملية تتضمن') || 'ملخصاً، مخاطر محتملة، وخطوة تالية.';

  const checklist = (() => {
    if (text.includes('رسائل') || text.includes('محادثة')) {
      return [
        'افتح المحادثة واقرأ آخر رسالتين قبل الرد حتى لا تفصل الإجابة عن سياقها.',
        'حدد هل الرسالة تطلب معلومة، مستنداً، موافقة، موعداً، أو إجراءً مالياً.',
        'اكتب رداً قصيراً يتضمن الإجراء التالي والمهلة المتوقعة.',
      ];
    }
    if (text.includes('وثيقة') || text.includes('توقيع')) {
      return [
        'تحقق من اسم الوثيقة، أطرافها، التاريخ، والملف المرتبطة به قبل التوقيع.',
        'راجع هل يوجد نقص في المرفقات أو شرط يحتاج توضيحاً مكتوباً.',
        'لا تغلق الإجراء قبل حفظ نسخة واضحة داخل ملف القضية.',
      ];
    }
    if (text.includes('إغلاق')) {
      return [
        'تأكد من أن الأتعاب مدفوعة وأن الوثائق المطلوبة لا تحتوي إجراءً معلقاً.',
        'راجع آخر محادثة للتأكد من عدم وجود طلب مفتوح من العميل أو المحامي.',
        'اكتب رسالة ختامية توثق سبب الإغلاق وما تم إنجازه.',
      ];
    }
    if (text.includes('مستندات') || text.includes('مستند')) {
      return [
        'ابدأ بالمستند الذي يثبت العلاقة القانونية أو الواقعة الأساسية.',
        'أضف أي مراسلات، إيصالات، صور، عقود، أو إنذارات مرتبطة بالنزاع.',
        'سم كل ملف باسم واضح حتى يسهل على المحامي تقييمه بسرعة.',
      ];
    }
    if (text.includes('دفعة') || text.includes('دفع')) {
      return [
        'راجع رقم الفاتورة وحالتها قبل إرسال تذكير بالدفع.',
        'استخدم صياغة مهذبة تحدد المبلغ والسبب والمهلة.',
        'لا تصعد الإجراء قبل التأكد من عدم وجود دفعة حديثة قيد المعالجة.',
      ];
    }
    if (text.includes('ملفك') || text.includes('البيانات الناقصة')) {
      return [
        'أكمل البيانات التي تزيد الثقة أولاً: رقم الهاتف، الموقع، الصورة، والتخصص عند المحامين.',
        'اكتب نبذة قصيرة تشرح الخبرة أو الحاجة القانونية بلغة مباشرة.',
        'راجع الملف بعد الحفظ لأن التوصيات تصبح أدق كلما كانت البيانات أوضح.',
      ];
    }
    return [
      'حوّل التنبيه إلى مهمة واحدة واضحة قابلة للتنفيذ الآن.',
      'اجمع أي معلومات ناقصة قبل اتخاذ قرار نهائي.',
      'وثق الخطوة التالية داخل الملف أو المحادثة حتى يبقى المسار واضحاً.',
    ];
  })();

  const suggestedMessage = (() => {
    if (text.includes('رسائل') || text.includes('محادثة')) {
      return 'مرحباً، اطلعت على التحديث. سأراجع النقاط المطلوبة الآن، وإذا كانت هناك وثيقة أو معلومة ناقصة سأرسلها ضمن هذا الملف لتبقى المتابعة موثقة.';
    }
    if (text.includes('إغلاق')) {
      return 'مرحباً، بما أن الملف يبدو مكتملاً، سأراجع آخر الوثائق والمدفوعات ثم أؤكد الإغلاق النهائي مع توثيق ما تم إنجازه.';
    }
    if (text.includes('دفعة') || text.includes('دفع')) {
      return 'مرحباً، نود تذكيركم بوجود دفعة تحتاج مراجعة. يرجى تأكيد حالة الدفع أو إرسال ما يثبت السداد حتى نستكمل الإجراء دون تأخير.';
    }
    return 'مرحباً، سأتابع هذه النقطة الآن وأرتب المتطلبات الناقصة، ثم أرسل تحديثاً واضحاً بالخطوة التالية داخل الملف.';
  })();
  ...
  return [
    ...
    '',
    `**السياق**: ${context}`,
    `**المطلوب الآن**: ${requestedStep}`,
    `**شكل الإجابة المطلوب**: ${expected}`,
    '',
    '**الخلاصة العملية**',
    'هذا التنبيه يحتاج متابعة منظمة أكثر من بحث قانوني عام. الأفضل تحويله إلى إجراء واضح داخل الملف، ثم توثيق الرد أو المستند أو قرار الإغلاق حتى لا تضيع المتابعة.',
    '',
    '**قائمة تحقق سريعة**',
    ...
    '',
    '**تنبيه مهني**',
    'هذه مساعدة تنظيمية داخل المنصة وليست رأياً قانونياً نهائياً. إذا كان الإجراء يمس حقاً جوهرياً أو موعداً قانونياً، راجع المحامي قبل الإرسال أو الإغلاق.',
  ].join('\n');
};
...
توقيع الطرف الثاني (المشتري): ............................`;
  ...
  app.post('/api/legal/ask', async (req, res) => {
    ...
    try {
      ...
      const platformAnswer = buildPlatformIntelligenceAnswer(question);
      if (platformAnswer) {
        return res.json({
          question,
