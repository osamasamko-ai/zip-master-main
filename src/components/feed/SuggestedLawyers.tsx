  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:hidden">
      {lawyers.slice(0, 2).map((lawyer) => (
        <div key={lawyer.id} className="flex items-center justify-between rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          <button onClick={() => onFollow(lawyer.id)} className="rounded-xl bg-brand-navy px-3 py-2 text-[11px] font-black text-white">
            متابعة
          </button>
